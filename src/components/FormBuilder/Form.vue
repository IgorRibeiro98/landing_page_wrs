<template>
  <v-row>
    <v-col
      v-for="(formItem, index) in props.form"
      v-show="formItem.hide ?? true"
      :cols="formItem?.cols?.cols ?? 12"
    ...
    >
      ...
      <div :class="{ 'mb-1': !formItem.description }">
        ...
        <span
          class="text-subtitle-2 font-weight-semi-bold text-error pb-6"
      ...
      <slot
        :name="`item:${formItem.value}`"
        :props="getProps(formItem)"
        :on="formItem.on"
      >
        <component
          :is="components[formItem.component]"
          v-model="form[formItem.value]"
          v-bind="getProps(formItem)"
          v-on="formItem.on ?? {}"
        />
      </slot>
    </v-col>
