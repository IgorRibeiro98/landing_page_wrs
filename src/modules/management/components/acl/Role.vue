<template>
  <View icon="mdi-shield-key-outline" :title="title">
    <v-form v-model="validate" lazy-validation @submit.prevent="save">
      <v-row>
        <v-col cols="12" md="3">
          <div class="mb-1">
            <span class="text-subtitle-2 font-weight-semi-bold pb-6 pr-1"
              >Nome do grupo</span
            >
            <span class="text-subtitle-2 font-weight-semi-bold text-error pb-6"
              >*</span
            >
          </div>
          <v-text-field
            ...
            :disabled="role.is_default ? true : false"
          ></v-text-field>
        </v-col>
        <v-spacer></v-spacer>
        <v-col cols="12" md="2" class="d-flex align-end justify-end">
          <v-btn color="primary" rounded="lg" type="submit"> Salvar </v-btn>
        </v-col>
      </v-row>
    </v-form>
    <v-row>
...
const title = computed(() =>
  role.value.id ? role.value.name : "Criar grupo de permissões"
);
...
const save = () => {
  if (!role.value.scopes.length) {
    openAlert(
      ...
      "É necessário selecionar pelo menos uma permissão"
    );
  }
